 ...
 * and two-phase locking discipline are followed.  <p>
 *
 * <u> Locking note: </u>
 * <p>
 * <p>
 * Many of the methods here are synchronized (to prevent concurrent log writes from happening); many
 * of the methods in BufferPool are also synchronized (for similar reasons.)  Problem is that
 * BufferPool writes log records (on page flushed) and the log file flushes BufferPool pages (on
 * checkpoints and recovery.)  This can lead to deadlock.  For that reason, any LogFile operation
 * that needs to access the BufferPool must not be declared synchronized and must begin with a block
 * like:
 *
 * <p>
 * <pre>
