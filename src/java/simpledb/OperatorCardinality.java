...
public class OperatorCardinality {
    ...
    if (o instanceof Filter) {
      ...
      if (children.length > 0 && children[0] != null) {
        if (children[0] instanceof Operator) {
          ...
          childC = tableStats.get(((SeqScan) children[0]).getTableName())
  ...
  private static boolean updateJoinCardinality(Join j, Map<String, Integer> tableAliasToId,
    ...
    if (child1 instanceof Operator) {
      ...
      child1Card = (int) (tableStats.get(((SeqScan) child1).getTableName())
          .estimateTableCardinality(1.0));
    }
    ...
    if (child2 instanceof Operator) {
      ...
      child2Card = (int) (tableStats.get(((SeqScan) child2).getTableName())
          .estimateTableCardinality(1.0));
    }
    ...
    j.setEstimatedCardinality(JoinOptimizer
        .estimateTableJoinCardinality(j.getJoinPredicate().getOperator(), tableAlias1, tableAlias2,
            pureFieldName1, pureFieldName2, child1Card, child2Card, child1HasJoinPK,
  ...
  private static boolean updateHashEquiJoinCardinality(HashEquiJoin j,
      Map<String, Integer> tableAliasToId, Map<String, TableStats> tableStats) {
  ...
  private static boolean updateAggregateCardinality(Aggregate a,
    ...
    if (tableId != null) {
      groupFieldAvgSelectivity = tableStats.get(Database.getCatalog().getTableName(tableId))
          .avgSelectivity(
              Database.getCatalog().getTupleDesc(tableId).fieldNameToIndex(pureFieldName),
              Predicate.Op.EQUALS);
