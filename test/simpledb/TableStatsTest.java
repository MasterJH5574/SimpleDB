...
public class TableStatsTest extends SimpleDbTestBase {
  ...
  /**
   * Verify the cost estimates of scanning various numbers of pages from a HeapFile This test checks
   * that the estimateScanCost is: +linear in numPages when IO_COST is constant +linear in IO_COST
   * when numPages is constant +quadratic when IO_COST and numPages increase linearly.
   */
  @Test
  ...
  /**
   * Verify that selectivity estimates do something reasonable. Don't bother splitting this into N
   * different functions for each possible Op because we will probably catch any bugs here in
  ...
  public void estimateSelectivityTest() {
    ...
    for (int col = 0; col < 10; col++) {
      ...
      Assert.assertEquals(0.5, s.estimateSelectivity(col, Predicate.Op.GREATER_THAN, halfMaxMin),
          0.1);
      Assert.assertEquals(31.0 / 32.0, s.estimateSelectivity(col, Predicate.Op.GREATER_THAN, atMin),
          0.05);
      ...
      Assert
          .assertEquals(0.0, s.estimateSelectivity(col, Predicate.Op.GREATER_THAN_OR_EQ, aboveMax),
              0.001);
      ...
      Assert.assertEquals(1.0, s.estimateSelectivity(col, Predicate.Op.GREATER_THAN_OR_EQ, atMin),
          0.015);
      Assert
          .assertEquals(1.0, s.estimateSelectivity(col, Predicate.Op.GREATER_THAN_OR_EQ, belowMin),
              0.001);
      ...
      Assert.assertEquals(1.0, s.estimateSelectivity(col, Predicate.Op.LESS_THAN_OR_EQ, atMax),
          0.015);
      Assert.assertEquals(0.5, s.estimateSelectivity(col, Predicate.Op.LESS_THAN_OR_EQ, halfMaxMin),
          0.1);
      Assert
          .assertEquals(0.0, s.estimateSelectivity(col, Predicate.Op.LESS_THAN_OR_EQ, atMin), 0.05);
