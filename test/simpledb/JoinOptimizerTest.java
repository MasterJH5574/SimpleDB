...
public class JoinOptimizerTest extends SimpleDbTestBase {
  ...
  /**
   * Verify that the estimated join costs from estimateJoinCost() are reasonable we check various
   * order requirements for the output of estimateJoinCost.
   */
  @Test
   ...
   */
  @Test
  public void orderJoinsTest()
      throws ParsingException, IOException, DbException, TransactionAbortedException {
  ...
  public void bigOrderJoinsTest()
    ...
    HeapFile smallHeapFileA = SystemTestUtil
        .createRandomHeapFile(2, 100, Integer.MAX_VALUE, null, smallHeapFileTuples, "c");
  ...
  public void nonequalityOrderJoinsTest()
    ...
    Assert.assertTrue(
        result.get(result.size() - 1).t2Alias.equals("a") || result.get(result.size() - 1).t1Alias
            .equals("a"));
  }
}
