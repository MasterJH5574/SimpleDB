...
public class BTreeDeadlockTest extends SimpleDbTestBase {
  ...
  /**
   * Helper method to clean up the syntax of starting a BTreeWriter thread. The parameters pass
   * through to the BTreeWriter constructor.
   */
   ...
   * <p>
   * This test causes two different transactions to update two (probably) different leaf nodes Each
   * transaction can happily insert tuples until the page fills up, but then it needs to obtain a
   * write lock on the root node in order to split the page. This will cause a deadlock situation.
   */
  @Test
  public void testReadWriteDeadlock() throws Exception {
    ...
    Database.getBufferPool()
        .getPage(tid1, BTreeRootPtrPage.getId(bf.getId()), Permissions.READ_ONLY);
    Database.getBufferPool()
        .getPage(tid2, BTreeRootPtrPage.getId(bf.getId()), Permissions.READ_ONLY);
