...
public class LockingTest extends TestUtil.CreateHeapFile {
   ...
   */
  public void metaLockTester(TransactionId tid1, PageId pid1, Permissions perm1, TransactionId tid2,
      PageId pid2, Permissions perm2, boolean expected) throws Exception {
   ...
   */
  public void grabLock(TransactionId tid, PageId pid, Permissions perm, boolean expected)
      throws Exception {
  ...
  /**
   * Unit test for BufferPool.getPage() assuming locking. Acquires a read lock and a write lock on
  ...
  /**
   * Unit test for BufferPool.getPage() assuming locking. Acquires a write lock and a read lock on
   * the same page, in that order.
   */
  @Test
  ...
  /**
   * Unit test for BufferPool.getPage() assuming locking. A single transaction should be able to
   * acquire a read lock after it already has a write lock.
   */
  @Test
  ...
  /**
   * Unit test for BufferPool.getPage() and BufferPool.releasePage() assuming locking. Acquires read
   * locks on different pages.
   */
  @Test
