...
public class BTreeFileDeleteTest extends SimpleDbTestBase {
  ...
  public void testStealFromLeftLeafPage() throws Exception {
    ...
    assertTrue(
        page.getNumTuples() == totalTuples / 2 || page.getNumTuples() == totalTuples / 2 + 1);
    assertTrue(
        sibling.getNumTuples() == totalTuples / 2 || sibling.getNumTuples() == totalTuples / 2 + 1);
  ...
  public void testStealFromLeftInternalPage() throws Exception {
    ...
    BTreeInternalPage page = BTreeUtility
        .createRandomInternalPage(pageId, keyField, BTreePageId.LEAF,
    ...
    assertTrue(
        page.getNumEntries() == totalEntries / 2 || page.getNumEntries() == totalEntries / 2 + 1);
    assertTrue(sibling.getNumEntries() == totalEntries / 2
        || sibling.getNumEntries() == totalEntries / 2 + 1);
    ...
    assertTrue(parent.iterator().next().getKey()
        .compare(Op.LESS_THAN_OR_EQ, page.iterator().next().getKey()));
    assertTrue(parent.iterator().next().getKey()
  ...
  public void testStealFromRightInternalPage() throws Exception {
    ...
    BTreeInternalPage sibling = BTreeUtility.createRandomInternalPage(siblingId, keyField,
        BTreePageId.LEAF, BTreeUtility.MAX_RAND_VALUE / 2, BTreeUtility.MAX_RAND_VALUE,
        4 + entriesPerPage / 2);
    ...
    // is the parent key reasonable?
    assertTrue(parent.iterator().next().getKey()
        .compare(Op.LESS_THAN_OR_EQ, sibling.iterator().next().getKey()));
    assertTrue(parent.iterator().next().getKey()
  ...
  public void testMergeInternalPages() throws Exception {
    ...
    int entriesPerPage = BTreeUtility.getNumEntriesPerPage();
    BTreeFile empty = BTreeUtility
        .createEmptyBTreeFile(emptyFile.getAbsolutePath(), 2, 0, 1 + 2 * entriesPerPage);
    int tableid = empty.getId();
    int keyField = 0;
    ...
    BTreeInternalPage leftPage = BTreeUtility
        .createRandomInternalPage(leftPageId, keyField, BTreePageId.LEAF,
            entriesPerPage / 2 - 1, 0, BTreeUtility.MAX_RAND_VALUE / 2, 3 + entriesPerPage);
    BTreeInternalPage rightPage = BTreeUtility
        .createRandomInternalPage(rightPageId, keyField, BTreePageId.LEAF,
            entriesPerPage / 2 - 1, BTreeUtility.MAX_RAND_VALUE / 2, BTreeUtility.MAX_RAND_VALUE,
